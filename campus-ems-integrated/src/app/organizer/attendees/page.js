...
export default function OrganizerAttendeesPage() {
  ...
  // Load all organizer events for the selector
  useEffect(() => {
    const load = async () => {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--bg-primary)' }}>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
        <DashboardHeader user={user} onMenuClick={() => setSidebarOpen(true)} title="Attendees" />
        ...
        {eventsLoading ? (
          ...
          <div className="card p-6 text-center">
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Create an event first to view attendees</p>
          </div>
        ) : (
