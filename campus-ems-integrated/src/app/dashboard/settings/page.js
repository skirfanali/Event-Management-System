...
export default function UserSettingsPage() {
  ...
  const [profile, setProfile] = useState({
    name: '', phone: '', college: '', branch: '', year: '', bio: ''
  });
        ...
        <div className="max-w-2xl mx-auto space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full mt-1 inline-block"
                    style={{ background: 'rgba(34,197,94,0.15)', color: '#22c55e' }}>STUDENT</span>
              ...
              <div className="flex items-center justify-between p-4 rounded-2xl"
                ...
                <div>
                  ...
                  <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Toggle theme</p>
                </div>
                <ThemeToggle />
              </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-2xl"
                style={{ background: 'var(--bg-tertiary)' }}>
                <div>
                  <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>Email Verified</p>
                  <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>{user?.email}</p>
                </div>
                <span className={`text-xs px-2 py-1 rounded-full font-medium ${user?.emailVerified ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500'}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  { label: 'College',     key: 'college', type: 'text', placeholder: 'Your institution'     },
                ...
                <div>
                  <label className="text-xs font-medium block mb-1.5" style={{ color: 'var(--text-secondary)' }}>Year</label>
                  <select className={inp} value={profile.year}
                    onChange={e => setProfile(p => ({ ...p, year: e.target.value }))}>
                    <option value="">Select year</option>
                    {['1st Year','2nd Year','3rd Year','4th Year','PG','Alumni'].map(y => (
          ...
          {activeTab === 'notifications' && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              className="card p-6 space-y-4">
              {[
                { label: 'Event reminders',   desc: '24 hours before an event you registered for' },
                { label: 'Ticket confirmed',  desc: 'When your registration is confirmed'          },
                { label: 'Payment receipts',  desc: 'After successful payment'                     },
                { label: 'New events nearby', desc: 'Events matching your interests'               },
              ].map((item, i) => (
